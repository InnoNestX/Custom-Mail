...
(() => {
  ...
  const attachPreviewUrls = [];

  function revokeAttachPreviewUrls() {
    for (const url of attachPreviewUrls) URL.revokeObjectURL(url);
    attachPreviewUrls.length = 0;
  }

  const ALLOWED_IMAGE_TYPES = new Set(["image/png", "image/jpeg", "image/gif", "image/webp"]);

  function attachmentPreviewUrl(f) {
    const type = String(f.type || "").toLowerCase();
    if (!ALLOWED_IMAGE_TYPES.has(type)) return null;
    try {
      const binary = atob(String(f.content).replace(/\s/g, ""));
  ...
  function renderAttachments() {
    ...
    for (let i = 0; i < pendingFiles.length; i++) {
      ...
      const previewUrl = attachmentPreviewUrl(f);
      if (previewUrl) {
  ...
  function appendPreviewMetaRow(parent, label, value) {
    const row = document.createElement("div");
  ...
  function openPreviewModal(data) {
    ...
    appendPreviewMetaRow(
      previewMeta,
      "From",
      String(data.fromName) + " <" + String(data.fromEmail) + ">",
    );
    appendPreviewMetaRow(previewMeta, "To", (data.to || []).join(", "));
    appendPreviewMetaRow(previewMeta, "Subject", data.subject);
    if (pendingFiles.length) {
      const attachLabel = document.createElement("div");
      const attachStrong = document.createElement("strong");
      attachStrong.textContent = "附件";
      attachLabel.appendChild(attachStrong);
      previewMeta.appendChild(attachLabel);
      const chipsWrap = document.createElement("div");
      chipsWrap.className = "preview-attach-chips";
      for (const f of pendingFiles) {
        const chip = document.createElement("span");
        chip.className = "preview-attach-chip";
        chip.textContent = f.name + " · " + formatSize(f.size);
        chipsWrap.appendChild(chip);
      }
      previewMeta.appendChild(chipsWrap);
    }
